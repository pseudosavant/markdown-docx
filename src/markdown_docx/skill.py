...
Use ATX and Setext headings, paragraphs, emphasis, strong text, strikethrough, superscript, subscript, inline code, hard line breaks, fenced and indented code blocks, blockquotes, thematic breaks, lists, pipe tables, links, and local or remote images. Write `[link text](https://example.com)` or a bare URL for native editable Word hyperlinks. Use `~~text~~` for strikethrough, `x^2^` for superscript, and `H~2~O` for subscript. Link labels preserve formatting and optional titles become tooltips. Links work in paragraphs, headings, quotes, lists, and table cells. Reference links, angle-bracket autolinks, mailto links, relative file links, and linked inline images are supported wherever their content is allowed. Relative file links resolve from the output document. Destinations are not fetched. Empty link destinations keep the formatted label as ordinary text without a hyperlink. Every heading creates a bookmark. Link to its slug with `[Details](#details)`, including before the heading. Slugs use plain heading text and image labels, normalized to NFC and lowercase. Punctuation is removed except underscores and hyphens. Whitespace becomes a hyphen. Empty slugs use `section`. Duplicates receive `-1`, `-2`, and later available suffixes in document order. Unicode and percent-encoded fragments are supported. Fragments must match the slug exactly. Missing targets produce `internal_link_unresolved` with the source line. Word bookmark names are generated separately to fit Word constraints. Lists use their first ordered marker as the starting number, including zero. Each separate list has an independent sequence. Later marker numbers are ignored as in normal Markdown. Indented continuation paragraphs stay unnumbered and align with item text. Nested lists restart independently. Task markers `- [ ]` and `- [x]` at the start of list items become clickable Word check boxes. Ordered task items keep their numbers. Task items can mix with ordinary list items. Empty list items create blank numbered or bulleted Word paragraphs. List items may contain headings, code blocks, quotes, tables, and images. Indent their source under the list item. Nested blocks retain order and align with item text. Blockquotes may contain headings, lists, code blocks, tables, images, and nested quotes. Nested blocks keep their own Word styles and gain quote indentation. A lone `>` creates an empty Quote paragraph. Thematic breaks create Word horizontal rules as empty paragraphs with bottom borders. Ordinary HTML comments are ignored. List styles must reference native template numbering. Pipe-table header rows repeat automatically when a table spans pages in Word. Body rows are not marked as headers. No Markdown directive is required. The underlying library exposes `row.repeat_as_header` for callers that need explicit control. Footnotes use the named `[^label]` and `[^label]: note text` extension from mdit-py-plugins. Labels are case-sensitive. Forward references, formatted text, links, hard breaks, and multiple note paragraphs are supported. Indent later paragraphs by four spaces. Each label must be defined and referenced exactly once. Empty notes are allowed. Undefined labels, duplicate definitions, unused definitions, and repeated references produce line-aware errors. Notes cannot contain images, lists, tables, headings, code blocks, metadata, or nested notes. References in image labels are rejected. References render as native Word notes with automatic numbering. No custom directive is required. Raw HTML tags and blocks and unsupported nested block content are rejected. Fenced code uses the first info-string word as a Pygments lexer alias. Recognized languages use the Pygments default light style as editable Word run colors and emphasis. Unknown or missing labels and indented code remain plain. Spaces, tabs, and internal blank lines are preserved.
